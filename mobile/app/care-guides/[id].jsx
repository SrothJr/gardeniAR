...
export default function CareGuideDetail() {
  ...
  const [activeSeason, setActiveSeason] = useState('Summer'); // Default to Summer or calculate current
  ...
  // SMART LOGIC: Find rule matching Stage AND (Season OR 'All Year')
  const getRule = (list) => {
    if (!list || list.length === 0) return null;
    
    // 1. Try Exact Match (Stage + Season)
    const exact = list.find(r => r.lifeStage === activeStage && (Array.isArray(r.season) ? r.season.includes(activeSeason) : r.season === activeSeason));
    if (exact) return exact;

    // 2. Try Stage + 'All Year'
    const allYear = list.find(r => r.lifeStage === activeStage && (Array.isArray(r.season) ? r.season.includes('All Year') : r.season === 'All Year'));
    if (allYear) return allYear;

    // 3. Fallback: Try 'General' Stage + Season
    const generalSeason = list.find(r => r.lifeStage === 'General' && (Array.isArray(r.season) ? r.season.includes(activeSeason) : r.season === activeSeason));
    if (generalSeason) return generalSeason;

    // 4. Last Resort: 'General' + 'All Year'
    return list.find(r => r.lifeStage === 'General' && (Array.isArray(r.season) ? r.season.includes('All Year') : r.season === 'All Year'));
  };
          ...
          <View style={styles.controlGroup}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.pillsScroll}>
              {SEASONS.map(season => (
                <TouchableOpacity 
                  key={season} 
                  style={[styles.pill, activeSeason === season && styles.pillActive]}
                  onPress={() => setActiveSeason(season)}
                >
                  <Text style={[styles.pillText, activeSeason === season && styles.pillTextActive]}>{season}</Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>
        ...
        <View style={styles.card}>
          ...
          {waterRule ? (
            <>
              ...
              {waterRule.description && <Text style={styles.desc}>{waterRule.description}</Text>}
            </>
          ) : (
            <Text style={styles.missing}>No data for {activeStage} in {activeSeason}</Text>
          )}
        </View>
        ...
        <View style={styles.card}>
          ...
          {fertRule ? (
            <>
              ...
              {fertRule.description && <Text style={styles.desc}>{fertRule.description}</Text>}
            </>
          ) : (
            <Text style={styles.missing}>No data for {activeStage} in {activeSeason}</Text>
          )}
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { color: '#94a3b8', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 },
  
  pills: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  pillsScroll: { flexDirection: 'row' },
  pill: { backgroundColor: '#0f1724', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, borderWidth: 1, borderColor: '#1e293b', marginRight: 8 },
  ...
  value: { color: '#e6eef3', fontWeight: '600', fontSize: 14, maxWidth: '65%', textAlign: 'right' },
